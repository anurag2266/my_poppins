...
const BookingList = ({ navigation }) => {
  ...
  const updateStatus = id => {
    ...
    console.log("booking ka",updateObj);
    helper
      .UrlReqAuthPost('api/nanny/booking_approval', 'POST', updateObj)
      .then(res => {
        console.log("booking approval", res);
        if (res.status) {
          global.global_loader_reff.show_loader(0);
                ...
                {newBooking.length > 0 && (
                  <View style={styles.innerCard}>
                    ...
                    {newBooking.map((items, index) => {
                      ...
                      console.log("items.ratting",items)
                      return (
                        <>
                          <View style={styles.detailView}>
                            ...
                            {items.booking_type === 1 ? (
                              ...
                              <Global.GlobalText
                              text={'Date - ' +  moment(items.from_date).format('DD/MM/YYYY') + ' TO ' +  moment(items.to_date).format('DD/MM/YYYY')}
                            ...
                            {/* {items.booking_type === 1 ? (
                              ...
                              <Global.GlobalText
                                text={'Time - ' + items.from_time + '' + items.to_time}
                                style={styles.message}
                              />
                            ) : null}
                            {items.booking_type === 2 ? ( */}
                              <>
                                {items.is_monday_available ? (
                                  <Global.GlobalText
                                    text={'Monday Time - ' + items.monday_from_time + ' ' + items.monday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                {items.is_tuesday_available ? (
                                  <Global.GlobalText
                                    text={'Tuesday Time - ' + items.tuesday_from_time + ' ' + items.tuesday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                {items.is_wednesday_available ? (
                                  <Global.GlobalText
                                    text={'Wednesday Time - ' + items.wednesday_from_time + ' ' + items.wednesday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                {items.is_thursday_available ? (
                                  <Global.GlobalText
                                    text={'Thuresday Time - ' + items.thursday_from_time + ' ' + items.thursday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                ...
                                {items.is_friday_available ? (
                                  <Global.GlobalText
                                    text={'Friday Time - ' + items.friday_from_time + ' ' + items.friday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                {items.is_saturday_available ? (
                                  <Global.GlobalText
                                    text={'Saturday Time - ' + items.saturday_from_time + ' ' + items.saturday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                                {items.is_sunday_available ? (
                                  <Global.GlobalText
                                    text={'Sunday Time - ' + items.sunday_from_time + ' ' + items.sunday_to_time}
                                    style={styles.message}
                                  />
                                ) : null}
                              </>
                  ...
                  {completeBooking.map(item => {
                    ...
                    return (
                      <View style={styles.detailView}>
                        ...
                        {!item.is_review ? (
                          <View
                            style={{ flexDirection: 'row', alignItems: 'center', marginVertical: scale(6), marginHorizontal: scale(10) }}>
                            <TouchableOpacity onPress={() => {
                              navigation.navigate('Reviews', { id: item.id, id_booking: item.id_booking })
                            }}>
                              <Global.GlobalText
                                text="Give Review"
                                style={[styles.clickHere, { textTransform: 'none', fontSize: scale(19), textDecorationLine: 'underline' }]}
                              />
                            </TouchableOpacity>
                          </View>
                        ) : (
