...
const Avability = ({ route, navigation }) => {
  ...
  const GetStatus = () => {
    helper.UrlReqAuth('api/nanny/is_profile_complete').then(res => {
      console.log("sddsdsdsds",res.status)
  ...
  const GetSingleAvailability = idEdit => {
    ...
    helper
      ...
      .then(res => {
        console.log("DATAAAA",'api/nanny/get_availability?month=' + selectMonth + '&id=' + idEdit )
        if (res.status) {
  ...
  const SaveAvability = () => {
    ...
    for (let i = 0; i < datedata.length; i++) {
      ...
      mark_as_off_array.push(
        datedata[i].is_mark_visible === true ? 'No' : 'Yes',
      );
    }
    ...
    console.log("date_array", signupObj)
    global.global_loader_reff.show_loader(1);
    helper
  ...
  const DateApiCall = month => {
    ...
    helper
      ...
      .then(res => {
        console.log('api/nanny/get_date?month=' + month + '&year=' + selectedYear)
        if (res.status) {
          global.global_loader_reff.show_loader(0);
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  if (item.is_mark_visible) {
                    Global.showError('Please Remove Mark As Off');
                  } else {
                    clickFromTime(index);
                  }
                }}
